...
function StudyRoomsView() {
  ...
  const { data, isLoading, error } = useStudyRooms(DATE_TABS[selectedIndex].dateStr)
  ...
  const renderContent = (): React.ReactNode => {
    if (isLoading) {
      return <p className={styles.loading}>조회 중...</p>
    }
      ...
      <div className={styles.dateTabs}>
        {DATE_TABS.map((tab, index) => (
          <button
            key={tab.dateStr}
            type="button"
            className={joinClassNames(styles.dateTab)}
